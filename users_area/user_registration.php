    ...
    <div class="col-lg-12 col-xl-6">
        <form action="" method="post" enctype="multusernameart/form-data">
            ...
            <div class="mt-4 pt-2">
                ...
                <p class="small fw-bold mt-2 pt-1 mb-0">Aleardy have an account ?<a href="user_login.php" class="text-danger"> Login</p>
            </div>
         </form>
...
if (!$result) {
    die("Query failed: " . mysqli_error($con)); // Log SQL errors
}

$rows_count = mysqli_num_rows($result);
if ($rows_count > 0) {
    echo "<scrusernamet>alert('Username and Email already exist')</scrusernamet>";
}else if($user_password!=$conf_user_password){
    echo "<scrusernamet>alert('Password do not match')</scrusernamet>";
}
 else {
    // insert query
    move_uploaded_file($user_image_tmp,"./user_images/$user_image");
    $insert_query = "INSERT INTO user_table (username, user_email, user_password,user_image, user_address, user_mobile, user_username)
                     VALUES ('$user_username', '$user_email', '$hash_password','$user_image','$user_address', '$user_contact','$user_username')";
    $sql_execute = mysqli_query($con, $insert_query);

    if ($sql_execute) {
        echo "<scrusernamet>alert('User registered successfully')</scrusernamet>";
    } else {
...
if($rows_count>0){
    ...
    echo "<scrusernamet>alert('You have items in your cart')</scrusernamet>";
    echo "<scrusernamet>window.open('checkout.php','_self')</scrusernamet>";
}else{
